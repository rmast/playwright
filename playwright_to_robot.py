...
class PlaywrightToRobotConverter:
    ...
    def _extract_selector(self, line: str) -> Optional[str]:
        ...
        def _format_has_text(expression: Optional[str]) -> Optional[str]:
            ...
            if string_match:
                text = _unescape(string_match.group(1))
                return f'internal:has-text="{_escape_text(text)}"i'
            ...
            if regex_match:
                pattern, flag = regex_match.groups()
                suffix = "i" if flag == "IGNORECASE" else ""
                return f'internal:has-text=/{_escape_regex(pattern)}/{suffix}'
        ...
        locator_patterns = [
            (
                re.compile(
                    r'locator\(["\']([^"\'\\]*(?:\\.[^"\'\\]*)*)["\'](?:,\s*has_text=(re\.compile\(.*?\)|["\'][^"\']*(?:\\.[^"\']*)*["\']))?\)'
                ),
                lambda s, has_text=None: [s] + ([_format_has_text(has_text)] if _format_has_text(has_text) else []),
            ),
            (
                re.compile(
        ...
        for pattern, formatter in locator_patterns:
            for match in pattern.finditer(line):
                groups = tuple(_unescape(group) for group in match.groups())
                step_matches.append((match.start(), formatter(*groups)))

        filter_pattern = re.compile(
            r'filter\(has_text=(re\.compile\(.*?\)|["\'][^"\']*(?:\\.[^"\']*)*["\'])\)'
        )
        for match in filter_pattern.finditer(line):
            formatted = _format_has_text(match.group(1))
            if formatted:
                step_matches.append((match.start(), [formatted]))

        nth_pattern = re.compile(r'\.nth\((-?\d+)\)')
        ...
        if step_matches:
            selectors = []
            for _, steps in sorted(step_matches, key=lambda item: item[0]):
                for step in steps:
                    if not selectors or selectors[-1] != step:
                        selectors.append(step)
    ...
    def _convert_select_option(self, action: Dict) -> str:
        ...
        selector = self._simplify_selector(action.get("selector", ""))
        value = action.get("value", "")
        return f"Select Options By{self.indent}{selector}{self.indent}label{self.indent}{value}"
